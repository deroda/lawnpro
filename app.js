...
function renderGrowthTrendChart() {
  ...
  if (pitchFilter === 'all' || pitchFilter === 'Pitch 1') {
    const p1Data = dates.map(d => {
      const item = clippingLogs.find(x => x.date === d && x.pitch === 'Pitch 1');
      return item ? item.estTotalL : 0;
    });
    datasets.push({
    ...
    datasets.push({
      label: `Pitch 1 Avg (${p1AvgVal.toFixed(1)} L)`,
  ...
  if (pitchFilter === 'all' || pitchFilter === 'Pitch 2') {
    const p2Data = dates.map(d => {
      const item = clippingLogs.find(x => x.date === d && x.pitch === 'Pitch 2');
      return item ? item.estTotalL : 0;
    });
    datasets.push({
    ...
    datasets.push({
      label: `Pitch 2 Avg (${p2AvgVal.toFixed(1)} L)`,
  ...
  charts.growthTrend = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af' }, title: { display: true, text: 'Liters (L)', color: '#9ca3af' } }
      }
    }
  });
}
  ...
  charts.clipVolBar = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          type: 'line',
          label: `Overall Avg ClipVol (${overallAvgCV.toFixed(3)} mL/m²)`,
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af' } },
        y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af' }, title: { display: true, text: 'ClipVol (mL/m²)', color: '#9ca3af' } }
      }
    }
  });
}
...
function renderComparisonStackedChart() {
  ...
  charts.stacked = new Chart(ctx, {
    ...
    data: {
      labels: dates,
      datasets: [
        { label: 'Pitch 1 Direct Harvest (L)', data: p1Totals, borderColor: '#ec4899', backgroundColor: 'rgba(236,72,153,0.1)', fill: true, tension: 0.3 },
        { label: 'Pitch 2 Direct Harvest (L)', data: p2Totals, borderColor: '#06b6d4', backgroundColor: 'rgba(6,182,212,0.1)', fill: true, tension: 0.3 },
        {
          label: `Overall Avg Harvest (${avgBasketVol.toFixed(1)} L)`,
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af' } },
        y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af' }, title: { display: true, text: 'Direct Basket Vol (L)', color: '#9ca3af' } }
      }
    }
  });
}
...
function renderAdvisoryTab() {
  ...
  p1List.innerHTML = '';
  if (p1Tot > 150) {
    p1List.innerHTML += `<li><i class="fa-solid fa-triangle-exclamation text-amber"></i> <strong>SURGE ALERT (>150L):</strong> Ryegrass in overdrive. Risk of root mass depletion & shear strength loss. Cut N dosing by 15-20%.</li>`;
    p1List.innerHTML += `<li><i class="fa-solid fa-scissors text-emerald"></i> Tighten PGR (Trinexapac-ethyl) intervals guided by GDD to prevent rebound flushes.</li>`;
  ...
  p2List.innerHTML = '';
  if (p2Tot > 150) {
    p2List.innerHTML += `<li><i class="fa-solid fa-triangle-exclamation text-amber"></i> <strong>SURGE ALERT:</strong> Rapid biomass production. Ryegrass may choke out fescue component.</li>`;
  } else if (p2Tot < 70) {
    ...
    p2List.innerHTML += `<li><i class="fa-solid fa-circle-check text-emerald"></i> <strong>OPTIMAL RANGE (70-150L):</strong> Ideal sward equilibrium for 50/50 PRG/SCRF mixture.</li>`;
    p2List.innerHTML += `<li><i class="fa-solid fa-arrows-spin text-emerald"></i> Alternate mowing direction (Width/Length) to prevent leaf blade lean & reduce thatch.</li>`;
    p2List.innerHTML += `<li><i class="fa-solid fa-scissors text-emerald"></i> Perform light verti-grooming to manage creeping red fescue rhizome organic build-up.</li>`;
  }
}
